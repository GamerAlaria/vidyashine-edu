...
if (!import.meta.env.VITE_WEB3FORMS_ACCESS_KEY) {
  console.error("SECURITY ALERT: VITE_WEB3FORMS_ACCESS_KEY is completely missing from the environment. Booking submissions will fail fast.");
}
...
const BookingModal = ({ isOpen, onClose }) => {
  ...
  const handleBook = async (e) => {
    ...
    if (!emailValue.endsWith('@gmail.com')) {
      alert('Please use a Gmail address (example@gmail.com) for booking.');
      setIsSubmitting(false);
      return;
    }
    if (!import.meta.env.VITE_WEB3FORMS_ACCESS_KEY || import.meta.env.VITE_WEB3FORMS_ACCESS_KEY === 'YOUR_WEB3FORMS_ACCESS_KEY_HERE') {
      alert("System Configuration Error: Web3Forms Access Key is missing.");
      setIsSubmitting(false);
      return;
    }
    ...
    const formData = {
      ...
      date_selected: `Feb ${selectedDate}, 2026`,
      message: `A new student has requested a free trial! \nName: ${e.target.elements.name.value}\nEmail: ${e.target.elements.email.value}\nRequested Date: Feb ${selectedDate}, 2026`
    };
    ...
    try {
      ...
      if (result.success) {
        ...
        alert("Failed to send booking. Please verify your Web3Forms access key in your .env file or App.jsx!");
        ...
        {step === 1 ? (
          <>
            ...
            <form onSubmit={handleBook} className="relative p-8 md:p-12 md:w-1/2 flex flex-col justify-center bg-[#F0F4FF]">
              ...
              <input name="email" required type="email" placeholder="Email Address (must be @gmail.com)" className="bg-white border border-primary/20 rounded-lg px-4 py-3 mb-8 font-data text-sm text-primary placeholder:text-primary/30 focus:outline-none focus:border-accent" disabled={isSubmitting} />
...
const Navbar = ({ onOpenModal }) => {
  ...
  return (
    <nav
      ref={navRef}
      className="fixed top-6 left-1/2 -translate-x-1/2 z-40 w-[95%] md:w-[90%] max-w-5xl rounded-[2rem] px-4 md:px-6 py-4 flex flex-col justify-between items-center transition-all duration-300 border border-transparent text-primary backdrop-blur-xl"
    >
      <div className="w-full flex justify-between items-center">
        ...
        <div className="hidden lg:flex gap-6 lg:gap-8 text-sm font-medium">
          {navLinks.map((link) => (
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden w-full pt-6 pb-2 mt-2 border-t border-primary/10 flex flex-col gap-4 animate-fade-in">
          ...
          <button onClick={() => { setMobileMenuOpen(false); onOpenModal(); }} className="mt-4 bg-accent text-white px-6 py-3 rounded-full text-sm font-medium max-w-[250px] text-center w-full shadow-[0_0_15px_rgba(37,99,235,0.5)]">
            Start free trial
          </button>
        </div>
      )}
...
const CANCEL_EMAIL = import.meta.env.VITE_SUPPORT_EMAIL || "your-email@example.com";
...
const Hero = ({ onOpenModal, onOpenCancelModal }) => {
  ...
  const milestones = [
    { cls: 'VI', label: 'Class VI', detail: 'Foundation building with NCERT-aligned Science & Math. Develops curiosity through real-world problem solving.' },
    { cls: 'VII', label: 'Class VII', detail: 'Advanced reasoning skills. Introduction to scientific methodology and algebraic thinking patterns.' },
    { cls: 'VIII', label: 'Class VIII', detail: 'Board-level preparation begins. Pre-foundation modules for future competitive aspirants.' },
    { cls: 'IX', label: 'Class IX', detail: 'Critical transition year. In-depth Physics, Chemistry & Math with integrated exam strategies.' },
    { cls: 'X', label: 'Class X', detail: 'Board excellence + competitive foundations. 200+ chapter-wise tests and national-level benchmarking.' },
    { cls: 'XI', label: 'Class XI', detail: 'Stream-specific deep dive. Integrated Board + JEE/NEET coaching with 300+ concept modules.' },
    { cls: 'XII', label: 'Class XII', detail: 'Final mastery. Board dominance + competitive peak performance. 500+ mock tests and mentor sessions.' },
  ];

  /* Competitive offerings data */
  const competitiveEdge = [
    { tag: 'JEE', desc: 'Engineering entrance mastery' },
    { tag: 'NEET', desc: 'Medical entrance dominance' },
    { tag: 'CUET', desc: 'University admission gateway' },
    { tag: 'NDA', desc: 'Defence academy preparation' },
  ];
  ...
  return (
    <section ref={heroRef} className="relative w-full min-h-[100dvh] flex flex-col overflow-hidden">
      ...
      {/* ===== NEW HERO CONTENT (z:5) — sits "inside" the particle atmosphere ===== */}
      <div className="relative w-full max-w-6xl mx-auto px-4 md:px-16 pt-24 pb-20 md:pt-32 md:pb-24 flex flex-col gap-20" style={{ zIndex: 5 }}>

        {/* ── Zone 1: Header ── */}
        <div className="hero-reveal">
          <h1 className="font-drama italic text-primary text-5xl md:text-8xl lg:text-9xl leading-none tracking-tight mb-6">
            What We Teach<span className="text-accent">.</span>
          </h1>
          <p className="font-data text-sm md:text-base text-primary/60 tracking-widest uppercase max-w-xl">
            A structured academic pathway from Class VI to XII — with integrated coaching for India's toughest competitive exams.
          </p>
        </div>
  ...
  return (
    <section className="py-32 px-6 md:px-16 w-full flex flex-col items-center justify-center z-10 relative mt-24">
      <div className="mb-28 text-center max-w-2xl">
        <h2 className="font-heading font-bold text-4xl tracking-tight text-primary mb-6">System Capabilities</h2>
        <p className="font-data text-primary/60 text-sm">Automated and disciplined performance optimization methodologies.</p>
      </div>
...
const DiagnosticShuffler = () => {
  const [cards, setCards] = useState([
    { id: 1, label: 'Structured Teaching', val: 'SYS.01' },
    { id: 2, label: 'Regular Testing', val: 'SYS.02' },
    { id: 3, label: 'Performance Tracking', val: 'SYS.03' }
  ]);
  ...
  return (
    <div className="bg-[#F0F4FF] border border-primary/15 rounded-[2rem] p-8 shadow-sm flex flex-col h-[400px]">
      <div className="flex justify-between items-start mb-8">
        <h3 className="font-heading font-bold text-xl text-primary">Score Improvement</h3>
        <span className="font-data text-xs text-accent">MODULE_A</span>
      </div>
      ...
      <p className="text-sm font-data text-primary/60 mt-8">Measurable and consistent outcomes.</p>
    </div>
  );
};
...
const TelemetryTypewriter = () => {
  ...
  const fullText = "Combining individual mentorship with a focused academic environment that drives disciplined performance.\n\n> AWAITING INPUT...";
    ...
    <div className="bg-[#EFF6FF] border border-primary/15 text-primary rounded-[2rem] p-8 shadow-[inset_0_0_20px_rgba(37,99,235,0.05)] flex flex-col h-[400px] relative overflow-hidden">
      <div className="flex justify-between items-center mb-8 pb-4 border-b border-primary/10">
        <div className="flex items-center gap-3">
          ...
          <span className="font-data text-xs tracking-widest text-primary/80">LIVE MENTORSHIP FEED</span>
        </div>
  ...
  return (
    <div className="bg-[#F0F4FF] border border-primary/15 rounded-[2rem] p-8 shadow-sm flex flex-col h-[400px] relative">
      <div className="flex justify-between items-start mb-6">
        <h3 className="font-heading font-bold text-xl text-primary">Academic Excellence</h3>
        <span className="font-data text-xs text-accent">SCHEDULER</span>
      </div>
      <p className="text-sm font-data text-primary/60 mb-8">Prepare not just to pass, but to compete.</p>
    ...
    <section ref={sectionRef} className="relative w-full py-40 px-6 md:px-16 overflow-hidden bg-[#0A1628] text-white min-h-screen flex flex-col items-center justify-center border-y border-primary/5">
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto flex flex-col gap-16 text-center items-center">
        <p className="manifesto-text font-heading text-xl md:text-3xl max-w-2xl text-white/50 text-center">
          Most general education focuses on: broad exposure and standardized repetition.
        </p>
        <h2 className="manifesto-text font-drama italic text-5xl md:text-8xl leading-tight text-white text-center">
          We focus on: <br />
          <span className="text-accent underline decoration-accent/30 underline-offset-8">measurable superiority.</span>
        </h2>
      </div>
    </section>
  );
  ...
  return (
    <section className="relative w-full bg-background pt-28 pb-32">
      <div className="px-6 md:px-16 w-full max-w-7xl mx-auto mb-24 relative z-10">
        <h2 className="font-heading font-bold text-4xl tracking-tight text-primary">The Methodology</h2>
      </div>
      ...
      <div className="relative w-full max-w-6xl mx-auto z-10 space-y-8">
        ...
        <div className="protocol-card min-h-[85vh] w-full bg-[#F0F4FF] border border-primary/15 rounded-[3rem] p-12 md:p-16 flex flex-col md:flex-row gap-12 md:gap-20 items-center shadow-[0_-10px_40px_rgba(10,22,40,0.08)] hover:border-primary/20 transition-all duration-300">
          <div className="flex-1">
            ...
            <h3 className="font-heading font-bold text-4xl md:text-5xl lg:text-6xl mb-8 text-primary">The Deep Assessment</h3>
            <p className="font-data text-base text-primary/70 leading-relaxed max-w-xl">
              We begin with a comprehensive diagnostic assessment designed to identify a learner's exact strengths and precise knowledge gaps. By mapping out an individualized academic baseline, we construct a clear, actionable pathway forward without assumptions.
            </p>
          </div>
        ...
        <div className="protocol-card min-h-[85vh] w-full bg-[#EFF6FF] border border-primary/15 rounded-[3rem] p-12 md:p-16 flex flex-col md:flex-row gap-12 md:gap-20 items-center shadow-[0_-20px_40px_rgba(10,22,40,0.08)] hover:border-primary/20 transition-all duration-300">
          <div className="flex-1">
            ...
            <h3 className="font-heading font-bold text-4xl md:text-5xl lg:text-6xl mb-8 text-primary">The Guided Learning</h3>
            <p className="font-data text-base text-primary/70 leading-relaxed max-w-xl">
              Once structural blind spots are identified, the system curates highly specific, high-yield resources to bridge those gaps. We replace redundant studying with targeted reinforcement, cementing core concepts through precision modules and expert mentorship.
            </p>
          </div>
          <div className="flex-1 flex justify-center items-center h-full min-h-[400px] relative">
        ...
        <div className="protocol-card min-h-[85vh] w-full bg-white text-primary border border-primary/15 rounded-[3rem] p-12 md:p-16 flex flex-col md:flex-row gap-12 md:gap-20 items-center shadow-[0_-20px_40px_rgba(10,22,40,0.1)] hover:border-primary/20 transition-all duration-300">
          <div className="flex-1">
            ...
            <h3 className="font-heading font-bold text-4xl md:text-5xl lg:text-6xl mb-8 text-primary">The Progress Tracking</h3>
            <p className="font-data text-base text-primary/70 leading-relaxed max-w-xl">
              Learning is verified constantly through dynamic progress tracking and authentic skill certification. We offer a transparent, real-time compass that visualizes student growth, ensuring every milestone translates into tangible academic confidence.
            </p>
          </div>
          <div className="flex-1 flex justify-center items-center h-full min-h-[400px] relative">
  ...
  return (
    <section className="w-full bg-background py-32 px-6 md:px-16 flex justify-center items-center relative z-10">
      <div className="bg-white border border-primary/5 rounded-[3rem] p-16 md:p-24 text-center max-w-4xl w-full flex flex-col items-center">
        <h2 className="font-heading font-bold text-4xl md:text-6xl mb-8 text-primary">Begin Your Journey.</h2>
        <p className="font-data text-primary/60 max-w-lg mb-14">Submit your application today to access our uncompromising academic environment.</p>
        ...
        <div className="space-y-8 font-data text-primary/80">
          ...
          <div>
            ...
            <p className="text-sm leading-relaxed">
              ...
              <span className="font-bold">Email:</span> your-privacy-email@example.com<br />
              <span className="font-bold">Phone:</span> YOUR_PHONE_NUMBER
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-8 font-data text-primary/80">
          ...
          <div>
            ...
            <p className="text-sm leading-relaxed">
              ...
              <span className="font-bold">Email:</span> your-support-email@example.com<br />
              <span className="font-bold">Phone:</span> YOUR_PHONE_NUMBER
            </p>
          </div>
        </div>
          ...
          <div className="flex-1 flex flex-col justify-center gap-8 bg-[#F0F4FF] border border-primary/5 rounded-[2rem] p-8 md:p-12 items-center text-center">
            ...
            <div className="flex flex-col items-center gap-4 w-full">
              ...
              <div className="text-center">
                <p className="font-data text-2xl text-primary/90 font-bold tracking-wide">YOUR_PHONE_NUMBER</p>
            ...
            <div className="h-96 overflow-hidden bg-gradient-to-b from-[#E0E7FF] to-[#F0F4FF] flex items-center justify-center">
              {!imageError ? (
                <img
                  ...
                  onLoad={() => console.log('HOD image loaded')}
                  onError={(e) => { console.error('HOD image failed', e); setImageError(true); }}
                  style={{ display: 'block' }}
                />
              ) : (
...
const Testimonials = () => {
  ...
  const reviews = [
    { type: "Student", text: "Vidyashine helped me improve my Maths and Physics concepts. Weekly tests really boosted my confidence.", author: "Rohan Sharma, Class 12" },
    { type: "Parent", text: "Very disciplined institute. Regular reports and tests helped track my child's progress.", author: "Mr. Rajesh Gupta (Parent of Class 10 Student)" },
    { type: "Student", text: "Best coaching for board + JEE foundation. Doubt sessions are very helpful.", author: "Ananya Gupta, Class 11" },
    { type: "Parent", text: "Faculty is very supportive and focused on concepts. Best coaching in our Noida area.", author: "Mrs. Neha Sharma (Parent of Class 12 Student)" },
    { type: "Student", text: "Small batch size and personal attention made a big difference in my scores.", author: "Aditya Verma, Class 10" },
    { type: "Parent", text: "Excellent environment for serious study and competitive preparation.", author: "Mr. Amit Verma (Parent of Class 11 Student)" }
  ];
  ...
  return (
    <section className="w-full bg-[#EFF6FF] py-36 border-t border-primary/5 relative z-10 overflow-hidden text-primary">
      <div className="w-full max-w-7xl mx-auto px-6 md:px-16 mb-24 text-center">
        <h2 className="font-heading font-bold text-4xl md:text-5xl tracking-tight text-primary mb-6">Voices of Success</h2>
        <p className="font-data text-primary/60 max-w-2xl mx-auto text-sm leading-relaxed">
          Excellence verified by the students and parents who trust our academic methodology.
        </p>
      </div>
                ...
                {blog.image ? (
                  <img
                    ...
                    onError={(e) => {
                      ...
                      e.target.parentNode.innerHTML += `<div class="w-full h-full flex items-center justify-center text-primary/20 bg-[#151515]"><svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-12 h-12 lucide lucide-terminal"><polyline points="4 17 10 11 4 5"/><line x1="12" x2="20" y1="19" y2="19"/></svg></div>`;
                    }}
                  />
                ) : (
